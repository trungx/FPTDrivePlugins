...
void frm:: startWork(char * haarcascade_frontalface_alt)
{
	 ...
	 // set up parameter :  
	  ...
	  while(!isRunning)
	  {
	    ...
	    cameraRecord.read(imageCaptureFull);
            if(imageCaptureFull.empty() ) break;  
	    int winX = 0;
	    int winY = 0;
...
// eyes blink checking
int frm::checkEyesBlinkStatus(Mat frame, bool& isBlink/*, CascadeClassifier *face_cascade*/)
{
	Mat gray_frame;
	...
	vector<Rect> faces;
        if (gray_frame.empty()) return -1;
	m_face_cascade->detectMultiScale(gray_frame, faces, 1.2, 2,
...
void frm::calculateCropROI(cv::Mat  imageCaptureFull, int& winX, int& winY, int& winW, int& winH)
{
	...
	float ImageOffsetInClient_X = (PBXCAMIMG_WIDTH - w / ImageScalingFactor_X) / 2;
	float ImageOffsetInClient_Y = (PBXCAMIMG_HEIGHT - h / ImageScalingFactor_Y) / 2;
	int wz = w / ScalingFactor;
	int hz = h / ScalingFactor;

	// duongvb hardcode for pbCamImg->Width = 192 and pbCamImg->Height = 144				
	int w_ins_tlx = MIN(PBXCAMIMG_WIDTH / 2 + wz / 2, MAX((PBXCAMIMG_WIDTH - wz) / 2, WIN_LEFT)) + 1;
	int w_ins_tly = MIN(PBXCAMIMG_HEIGHT  / 2 + hz / 2, MAX((PBXCAMIMG_HEIGHT - hz) / 2, WIN_TOP)) + 1;
	int w_ins_brx = MIN(PBXCAMIMG_WIDTH / 2 + wz / 2, MAX((PBXCAMIMG_WIDTH - wz) / 2, WIN_LEFT + WIN_WIDTH)) - 1;
	int w_ins_bry = MIN(PBXCAMIMG_HEIGHT / 2 + hz / 2, MAX((PBXCAMIMG_HEIGHT - hz) / 2, WIN_TOP + WIN_HEIGHT)) - 1;

	winleft = (int)((w_ins_tlx - ImageOffsetInClient_X)* ImageScalingFactor_X);
